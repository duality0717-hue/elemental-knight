...
for(const [name,setup] of Object.entries(states)){
 ...
 fs.writeFileSync(path.join(base,'.qa',name+'.html'),'<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>QA '+name+'</title><style>body{margin:0;background:#16151f}</style></head><body>'+html+'</body></html>');
}
